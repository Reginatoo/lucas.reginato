...
void desenhaTexto(FILE *f, double x, double y, char *texto) {
    fprintf(f, "<text x=\"%f\" y=\"%f\" font-family=\"Verdana\" font-size=\"5\" fill=\"black\">%s</text>\n", x, y, texto);
}

void desenhaX(FILE *f, double x, double y, char *cor) {
    fprintf(f, "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" stroke=\"%s\" stroke-width=\"1.5\" />\n", x - 4, y - 4, x + 4, y + 4, cor);
    fprintf(f, "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" stroke=\"%s\" stroke-width=\"1.5\" />\n", x - 4, y + 4, x + 4, y - 4, cor);
}
...
void desenhaCirculo(FILE *f, double x, double y, double r, char *fill, char *stroke) {
    fprintf(f, "<circle cx=\"%f\" cy=\"%f\" r=\"%f\" fill=\"%s\" stroke=\"%s\" stroke-width=\"1\" />\n", x, y, r, fill, stroke);
}